...
export default function HoverReceiver() {
        ...
        if (allMatchingElements.length > 0) {
          ...
          if (fam) {
            ...
            if (!loadedFontFamilies.current.has(familyKey)) {
              ...
              link.rel = "stylesheet";
              link.href = `https://fonts.googleapis.com/css2?family=${familyKey}`;
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        if (parent) {
          ...
          if (Math.abs(widthPercent - Math.round(widthPercent)) < 0.1 || [25, 33.333, 50, 66.667, 75, 100].some(v => Math.abs(widthPercent - v) < 0.5)) {
            widthValue = `${Math.round(widthPercent * 10) / 10}%`;
          }

          if (Math.abs(heightPercent - Math.round(heightPercent)) < 0.1 && [25, 50, 75, 100].includes(Math.round(heightPercent))) {
        ...
        if (hasMaxWidth) styles.maxWidth = widthValue;
        if (hasMaxHeight) styles.maxHeight = heightValue;
  ...
  useEffect(() => {
    function onPointerMove(e) {
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        allMatchingElements.forEach((element) => {
          ...
          if (elementId === focusedElementId) return;
          const rect = element.getBoundingClientRect();
    ...
    function onClickCapture(e) {
      ...
      if (hit) {
        ...
        if (hitId && typeof window !== "undefined") {
          localStorage.setItem(FOCUSED_ELEMENT_KEY, JSON.stringify({ id: hitId, tag: tagName }));
        }
        ...
        if (isEditable) {
          ...
          if (hit !== editingElementRef.current) {
            ...
            const createHandlers = (el) => {
              const handleFocus = () => {
                ...
                if (el.childElementCount > 0) {
                  originalContentRef.current = extractDirectTextContent(el);
                } else {
                  originalContentRef.current = el.innerText || el.textContent || "";
                }
                hasStyleChangesRef.current = false;
              };
            ...
            hit._editHandlers = { focus: handlers.handleFocus, blur: handlers.handleBlur, input: handlers.handleInput };
          }
        }

        const expandedBox = expandBox(hit.getBoundingClientRect());
        setFocusBox(expandedBox);
        setHoverBox(null);

        const className = hit.getAttribute("class") || "";
        const srcRaw = hit.tagName.toLowerCase() === "img" ? hit.src : null;
        ...
        const msg = {
          ...
          id: hitId,
          tag: tagName,
          rect: expandedBox || { top: 0, left: 0, width: 0, height: 0 },
    ...
    function onMsg(e) {
      ...
      if (e.data.msg === "PREVIEW_FONT") {
      ...
      } else if (e.data.msg === "SCROLL") {
        window.scrollBy(e.data.dx, e.data.dy);
      } else if (e.data.msg === "VISUAL_EDIT_MODE") {
      ...
      } else if (e.data.msg === "CLEAR_INLINE_STYLES") {
      ...
      } else if (e.data.msg === "SHOW_ELEMENT_HOVER") {
        ...
        if (allMatchingElements.length > 0) {
          ...
          let tagName = "";
          allMatchingElements.forEach((element) => {
            if (element === focusedElementRef.current) return;
            boxes.push(expandBox(element.getBoundingClientRect()));
            if (!tagName) tagName = element.getAttribute("data-orchids-name") || element.tagName.toLowerCase();
          });
          setHoverBoxes(boxes);
    ...
    function onScroll() {
      ...
      scrollTimeoutRef.current = window.setTimeout(() => {
        ...
        postMessageDedup({ type: CHANNEL, msg: "SCROLL_STOPPED" });
      }, 16);
    }
  ...
  return (
    <>
      <div
        className="fixed pointer-events-none z-[10000] border-2 border-blue-400 bg-blue-100/20 transition-[top,left,width,height] duration-75"
        style={{
          display: isVisualEditMode && hoverBox ? "block" : "none",
          top: hoverBox?.top ?? 0,
      ...
      >
        <div className="absolute top-0 left-0 -translate-y-full bg-blue-500 text-white text-[10px] px-1 py-0.5 rounded-t-sm whitespace-nowrap">
          {hoverTag}
        </div>
      </div>
      ...
      {hoverBoxes.map((box, i) => (
        <div
          key={i}
          className="fixed pointer-events-none z-[10000] border-2 border-blue-400 bg-blue-100/20"
          style={{
            display: isVisualEditMode ? "block" : "none",
            top: box?.top ?? 0,
            left: box?.left ?? 0,
            width: box?.width ?? 0,
            height: box?.height ?? 0,
      ...
      <div
        className="fixed pointer-events-none z-[10001] border-2 border-blue-600 bg-transparent transition-[top,left,width,height] duration-75"
        style={{
          display: isVisualEditMode && focusBox ? "block" : "none",
          top: focusBox?.top ?? 0,
          left: focusBox?.left ?? 0,
          width: focusBox?.width ?? 0,
          height: focusBox?.height ?? 0,
        }}
      >
        <div className="absolute top-0 left-0 -translate-y-full bg-blue-600 text-white text-[10px] px-1 py-0.5 rounded-t-sm whitespace-nowrap flex items-center gap-1">
          {focusTag}
        </div>

        <div
          className="resize-handle absolute -right-1 -bottom-1 w-2.5 h-2.5 bg-white border-2 border-blue-600 rounded-full cursor-nwse-resize pointer-events-auto"
          onMouseDown={(e) => handleResizeStart(e, "se")}
        />
        <div
          className="resize-handle absolute -right-1 top-1/2 -translate-y-1/2 w-2 h-4 bg-white border-2 border-blue-600 rounded-sm cursor-ew-resize pointer-events-auto"
          onMouseDown={(e) => handleResizeStart(e, "e")}
        />
        <div
          className="resize-handle absolute left-1/2 -translate-x-1/2 -bottom-1 w-4 h-2 bg-white border-2 border-blue-600 rounded-sm cursor-ns-resize pointer-events-auto"
